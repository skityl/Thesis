...
\begin{proposition}
    ...
    on the Von Neumann algebra $\M$. We denote the action as $\alpha:G\times\M\rightarrow\M$. Then there is a set 
    \begin{equation}
        \{ u(p) \;:\; p \in \widehat{G}\}
    \end{equation}
...
\begin{example}
    ...
    \end{equation}
    This is simply the $n$th Fourier coefficient times $\zeta^n$. Hence we have (up to a unimodular 
    scaling factor)
    \begin{equation}
        u(n)(\zeta) = \zeta^n.
    \end{equation}
...
$\M$ in the following sense, proved in \cite{Pedersen1979}:
\begin{proposition}
...
is called the Dirichlet kernel, and $F_N$ is the F\'jer kernel. See \cite{katznelson} for proofs.
